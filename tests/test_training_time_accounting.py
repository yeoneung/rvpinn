...
def write_manifest(tmp_path, suffix='1', seed='3', **updates):
    record = {
        'command': ('C:\\old workspace\\scripts\\train_pinn_pi.py '
                    '--tag study_winter_fee40 --region confirmatory '
    ...
    path = tmp_path / f'train_study_winter_fee40_{suffix}.json'
...
def test_training_run_time_includes_postfit_work_and_provenance(tmp_path):
    ...
    result = recorded_training_time(tmp_path, 'study_winter_fee40', 3, 225.0)
...
def test_training_run_requires_exact_single_seed(tmp_path, seed):
    ...
    with pytest.raises(ValueError, match='found 0'):
        recorded_training_time(tmp_path, 'study_winter_fee40', 3, 225.0)
...
def test_training_run_allows_only_timestamp_quantization_slack(tmp_path):
    write_manifest(tmp_path)
    assert recorded_training_time(tmp_path, 'study_winter_fee40', 3, 240.5)['training_wall_s'] == 240.0
    with pytest.raises(ValueError, match='cannot cover'):
        recorded_training_time(tmp_path, 'study_winter_fee40', 3, 241.1)
