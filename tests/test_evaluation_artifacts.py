...
def test_aggregation_requires_valid_search_for_learned_policies(tmp_path):
    ...
    base['action_search_version'] = ['incompatible-grid']
    pd.DataFrame(base).to_parquet(path)
    with pytest.raises(RuntimeError, match='mixed or incompatible action-search'):
        read_aligned_parquet(path)
...
def test_aggregation_rejects_incompatible_and_mixed_records(tmp_path):
    path = tmp_path / 'daily.parquet'
    pd.DataFrame({'common_cost': [1.]}).to_parquet(path)
    with pytest.raises(RuntimeError, match='not a valid'):
        read_aligned_parquet(path)
    pd.DataFrame({'deployment_version': [DEPLOYMENT_VERSION, 'incompatible']}).to_parquet(path)
    with pytest.raises(RuntimeError, match='mixed or incompatible'):
        read_aligned_parquet(path)
...
def test_parameter_scale_is_available_without_checkpoint_files():
    spec = importlib.util.spec_from_file_location('parameter_export', EXP / 'make_comparative_parameter_tables.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    scale = module._selected_scale('study_winter_fee40', 'winter_weekday', 0)
