"""Independent, read-only coverage and accounting audit of evaluation artifacts.
...
def selection_tariff(tag):
    central = re.fullmatch(r'study_(?:winter|spring|summer|autumn)_fee(\d+)', tag)
    factorial = re.fullmatch(r'study_sens_t(\d+)_f(\d+)_w(\d+)', tag)
    if central:
...
def expected_online_files():
    ...
    for stream in (7301, 7302, 7303):
        for fee, count, budget in ((40, 2, 5), (40, 8, 5), (40, 16, 5), (80, 16, 5), (80, 16, 15)):
            name = f'common_daily_confirmatory_thr300_fee{fee}_w10_n{count}_nested_stream{stream}_pool16_tl{budget}.parquet'
...
def check_online_parameters(frame, filename):
    ...
    if 'nested_stream' in suffix:
        stream = int(re.search(r'nested_stream(\d+)_pool16', suffix).group(1))
...
def main():
    ...
    main_cells = [(f'study_{season}_fee{fee}', f'{season}_weekday', fee, seeds)
        ...
        for fee in (20, 40, 80):
            for width in (5, 10, 20):
                if threshold == 300 and width == 10:
                    continue
                name = f'selected_daily_study_sens_t{threshold}_f{fee}_w{width}.parquet'
                expected_daily.add(name)
                path = results / name
                if not path.exists():
                    missing.append(name)
    ...
    name = 'comparative_reference_daily.parquet'
    expected_daily.add(name)
    ...
    for name in sorted(expected_daily):
        ...
        try:
            ...
            if name.startswith(('learned_', 'selected_', 'comparative_reference')):
    ...
    for path in sorted((workspace / 'checkpoints').glob('study_*/confirmatory/*/seed*/validation_selection.json')):
        audit = json.loads(path.read_text(encoding='utf-8'))
        if audit.get('action_search_version') != ACTION_SEARCH_VERSION:
    ...
    report = {'workspace': str(workspace), 'partial_mode': args.partial,
              'status': ('failed' if errors else 'partial_coverage' if args.partial or missing
