"""Reject incompatible or mixed numerical implementations during aggregation."""
...
def read_aligned_parquet(path):
    ...
    if frame.empty or 'deployment_version' not in frame:
        raise RuntimeError(f'not a valid deployment artifact: {path}')
    if not frame['deployment_version'].eq(DEPLOYMENT_VERSION).all():
        raise RuntimeError(f'mixed or incompatible deployment versions: {path}')
    candidate_artifact = Path(path).name.startswith((
        'learned_daily_', 'selected_daily_', 'comparative_reference_daily',
        'validation_daily'))
    if candidate_artifact:
        ...
        if not valid.all():
            raise RuntimeError(f'mixed or incompatible action-search versions: {path}')
    return frame
