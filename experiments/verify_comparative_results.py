"""Fail fast on incomplete or internally inconsistent comparison artifacts."""
...
from analyze_comparative_scenarios import COUNTS, STREAMS, paths
...
def verify_mechanism(results):
    metadata = json.loads((results / 'comparative_mechanism_day_metadata.json').read_text(encoding='utf-8'))
    summary = pd.read_csv(results / 'comparative_mechanism_day_summary.csv')
    trajectory = pd.read_parquet(results / 'comparative_mechanism_day_trajectory.parquet')
    check_mechanism_layout(metadata, summary, trajectory)
    from make_comparative_mechanism_figure import _check_reported_economics
    from src.deployment_numerics import DEPLOYMENT_VERSION
    for key in ('learned', 'reference', 'm2', 'm16'):
        record = json.loads((results / f'comparative_mechanism_{key}_summary.json').read_text(encoding='utf-8'))
        if (record.get('deployment_version') != DEPLOYMENT_VERSION
            ...
            raise RuntimeError('representative day lacks an input-source check')
        source_rows = []
        ...
        path = results / f'comparative_mechanism_{key}_trajectory.parquet'
...
def main() -> int:
    ...
    print("comparative artifact verification passed")
    return 0
