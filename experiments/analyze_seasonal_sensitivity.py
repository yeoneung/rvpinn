...
SEASON_TAGS = {
    "winter_weekday": "study_winter_fee40",
    "spring_weekday": "study_spring_fee40",
    "summer_weekday": "study_summer_fee40",
    "autumn_weekday": "study_autumn_fee40",
}
METHODS = (
    "no_storage",
        ...
        for fee in (20, 40, 80):
            ...
            for width in (5, 10, 20):
                if threshold == 300 and width == 10:
                    learned = learned_selected(f"study_winter_fee{fee}")
                    selected_batches = 5 if fee == 40 else 1
                    n_restarts = 25 if fee == 40 else 5
                else:
                    tag = f"study_sens_t{threshold}_f{fee}_w{width}"
...
def main() -> int:
    ...
    lines.extend([
        ...
        r"\centering",
        r"\small",
        (r"\caption{Compute-time payback against the online MIQP with lower "
         r"operating cost. Cost $\Delta$ is offline minus comparator in "
         r"EUR/day; fees are in EUR/h. The last column ignores operating-cost differences "
         r"and values offline and online compute seconds equally; it is not an economic break-even period.}"),
        r"\label{tab:supp-amortization}",
        r"\begin{tabularx}{\linewidth}{r>{\raggedright\arraybackslash}Xrrr}",
        r"\toprule",
        rf"Fee & Online comparator & Cost $\Delta$ & Saved s/day & \shortstack{{Compute-time\\payback (days)}} {br}",
        r"\midrule",
    ])
