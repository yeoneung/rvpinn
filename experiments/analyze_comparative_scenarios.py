...
def paths(fee: int, count: int, stream: int, budget: int = 5):
    suffix = f"nested_stream{stream}_pool16_tl{budget}"
...
def _write_latex(settings, streams, solvers, pairs) -> None:
    ...
    main_lines = [
        ...
        r"\label{tab:comparative-scenarios}",
    ...
    supp_lines = [
        ...
        r"\label{tab:supp-comparative-components}",
    ...
    supp_lines.extend([
        ...
        r"\label{tab:supp-comparative-solvers}",
        r"\resizebox{\linewidth}{!}{%",
    ...
    supp_lines.extend([
        ...
        r"\label{tab:supp-comparative-component-distribution}",
    ...
    (GENERATED / "comparative_scenarios.tex").write_text(
        "\n".join(main_lines), encoding="utf-8")
    (GENERATED / "comparative_scenario_supplement.tex").write_text(
...
def main() -> int:
    ...
    settings_frame.to_csv(RESULTS / "comparative_scenario_setting_summary.csv",
                          index=False)
    streams_frame.to_csv(RESULTS / "comparative_scenario_stream_summary.csv",
                         index=False)
    solvers_frame.to_csv(RESULTS / "comparative_scenario_solver_summary.csv",
                         index=False)
    pairs_frame.to_csv(RESULTS / "comparative_scenario_pairwise.csv", index=False)
    ...
    (RESULTS / "comparative_scenario_summary.json").write_text(
