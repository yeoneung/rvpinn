...
LEARNED = {
    20: "learned_daily_study_winter_fee20.parquet",
    40: "learned_daily_study_winter_fee40.parquet",
    80: "learned_daily_study_winter_fee80.parquet",
}
COMPONENTS = (
...
def _write_latex(methods: pd.DataFrame, pairs: pd.DataFrame) -> None:
    lines = [
        ...
        r"\toprule",
        r"Fee & Reference & Learned & Difference & Upper bound \\",
        r"\midrule",
    ]
    for fee in (20, 40, 80):
    ...
    ordered_pairs = pairs.sort_values('fee')

    def prose_list(values):
        return ", ".join(values[:-1]) + ", and " + values[-1]

    fees = prose_list([str(int(row.fee)) for row in ordered_pairs.itertuples()])
    differences = prose_list([f"{row.mean_diff:.1f}" for row in ordered_pairs.itertuples()])
    bounds = prose_list([f"{row.one_sided_95_upper:.1f}" for row in ordered_pairs.itertuples()])
    attribution = [
        f"At fee rates of {fees} EUR/h, learned-minus-reference costs were "
        f"{differences} EUR/day, respectively.",
        f"The corresponding one-sided upper bounds were {bounds} EUR/day.",
    ]
    supported = pairs.loc[pairs['one_sided_95_upper'] < 0, 'fee'].astype(int).tolist()
    if len(supported) == 3:
        overview = (
        ...
        overview = (
            r'No reference-comparison anchor had a one-sided upper bound below '
            r'zero (Table~\ref{tab:reference-attribution}).')
    else:
        overview = (
            'The one-sided upper bound was below zero at fee rates of '
    ...
    (GENERATED / "comparative_reference.tex").write_text(
    ...
    supplement = [
        ...
        r"\centering",
        r"\scriptsize",
        r"\caption{Exploratory learned-correction attribution by cost component. Fees are in EUR/h, cost components in EUR/day, throughput in kWh/day, and exceedance in hours/day. The central learned row averages five complete training batches.}",
    ...
    (GENERATED / "comparative_reference_supplement.tex").write_text(
        "\n".join(supplement), encoding="utf-8")


def main() -> int:
    reference = read_aligned_parquet(RESULTS / "comparative_reference_daily.parquet")
    if len(reference) != 90:
    ...
    methods.to_csv(RESULTS / "comparative_reference_method_summary.csv",
                   index=False)
    pairs.to_csv(RESULTS / "comparative_reference_pairwise.csv", index=False)
    batches.to_csv(RESULTS / "comparative_reference_batch_summary.csv",
                   index=False)
    ...
    (RESULTS / "comparative_reference_summary.json").write_text(
