...
FIGURE = ROOT / "manuscript" / "figures" / "fig_comparative_mechanism.pdf"
TEX = ROOT / "manuscript" / "generated" / "comparative_mechanism.tex"
...
def _learned_controller(p, prof):
    ...
    batch_path = RESULTS / "restart_batches_study_winter_fee40.json"
    ...
    if seed is None:
        ...
        return controller
    selection = (ROOT / "checkpoints" / "study_winter_fee40" / "confirmatory"
...
def _tex_table(summary: pd.DataFrame, date: str) -> str:
    ...
    lines = [
        ...
        r"\label{tab:comparative-mechanism}",
...
def _reported_day(key, date):
    if key == 'learned':
        path = RESULTS / 'learned_daily_study_winter_fee40.parquet'
        frame = read_aligned_parquet(path)
        frame = frame[frame['selected_by_batch'] & (frame['batch'] == 0)]
    elif key == 'reference':
        path = RESULTS / 'comparative_reference_daily.parquet'
        frame = read_aligned_parquet(path)
        frame = frame[frame['fee_per_hour'] == 40]
    elif key in ('m2', 'm16'):
        name = ('common_daily_confirmatory_thr300_fee40_w10_n2.parquet' if key == 'm2'
                else 'common_daily_confirmatory_thr300_fee40_w10_n16_nested_stream7301_pool16_tl5.parquet')
        path = RESULTS / name
        frame = read_aligned_parquet(path)
...
def _run_controller(key: str) -> None:
    ...
    trajectory_path = RESULTS / f"comparative_mechanism_{key}_trajectory.parquet"
    ...
    (RESULTS / f"comparative_mechanism_{key}_summary.json").write_text(
...
def _assemble() -> None:
    ...
    trajectory_rows = []
    for key in CONTROLLERS:
        summary_path = RESULTS / f"comparative_mechanism_{key}_summary.json"
        trajectory_path = RESULTS / f"comparative_mechanism_{key}_trajectory.parquet"
        ...
        if (row.get('deployment_version') != DEPLOYMENT_VERSION
            ...
            raise RuntimeError('representative-day artifacts require matching, verified input sources')
    ...
    summary.to_csv(RESULTS / "comparative_mechanism_day_summary.csv", index=False)
    trajectory.to_parquet(
        RESULTS / "comparative_mechanism_day_trajectory.parquet", index=False)
    metadata = {
        "date": date,
        "selection_rule": (
    ...
    (RESULTS / "comparative_mechanism_day_metadata.json").write_text(
