...
FEES = (20, 40, 80)
LEARNED_TAG = {
    20: "study_winter_fee20",
    40: "study_winter_fee40",
    80: "study_winter_fee80",
}
COMPARATORS = (
    "no_storage",
...
def render_abstract_result(methods, pairs):
    """Use the same named comparator for abstract cost and latency contrasts."""
    learned = methods[methods['method'] == 'offline_value_policy'].set_index('fee').loc[list(FEES)]
    stochastic = methods[
        methods['method'] == 'stochastic_two_stage_exact_band_miqp'
    ].set_index('fee').loc[list(FEES)]
    comparisons = [relative_cost_description(
        learned.loc[fee, 'mean_common_cost'], stochastic.loc[fee, 'mean_common_cost'])
        for fee in FEES]
    ...
    if bool(primary_pairs['superiority'].all()):
        ...
        inference_sentence = (
            "The prespecified learned-superiority rule was met in "
            f"{int(primary_pairs['superiority'].sum())} of six MIQP comparisons. ")
    return (
        "On 30 held-out 2019 winter weekdays, learned-policy cost was "
        f"{comparisons[0]}, {comparisons[1]}, and {comparisons[2]} relative to "
        "primary two-stage stochastic MIQP at fee rates of 20, 40, and 80 EUR/h, "
...
def main() -> int:
    ...
    fallback_sentence = (
        ...
        + ".")
    if fallback_by_fee.sum() == 0:
        fallback_sentence = (
            f"None of the {len(selections)} training batches retained "
    ...
    rule_sentence = (
        "Validation selected the following price-threshold rules: "
        + ", ".join(rule_choices)
        + ".")
    if all(choice.endswith(": zero action") for choice in rule_choices):
        rule_sentence = (
            "The price-threshold rule selected zero action at all three fee rates.")
    ...
    abstract = render_abstract_result(methods, pairs)
